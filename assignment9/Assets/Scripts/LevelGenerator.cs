...
public class LevelGenerator : MonoBehaviour {
	...
	public enum Blocks { WALL, FLOOR, HOLE };

	public int holes = Random.Range(3, 5);
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				} else if (!characterPlaced && mapData[z, x] != Blocks.HOLE) {
	...
	Blocks[,] GenerateMazeData() {
		Blocks[,] data = new Blocks[mazeSize, mazeSize];

		// initialize all walls to true
		for (int y = 0; y < mazeSize; y++) {
			for (int x = 0; x < mazeSize; x++) {
				data[y, x] = Blocks.WALL;
			}
		}
		...
		while (tilesConsumed < tilesToRemove) {
			...
			for (int i = 0; i < numSpacesMove; i++) {
				...
				if (data[mazeY, mazeX] == Blocks.WALL) {
					data[mazeY, mazeX] = Blocks.FLOOR;
					tilesConsumed++;
				}
			}
		}
		...
		while (holes > 0){
			...
			if (data[x,y] == Blocks.FLOOR){
				data[x,y] = Blocks.HOLE;
				holes--;
			}
		}
